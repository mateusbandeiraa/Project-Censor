...
public class Controller extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		case "editar":
			...
			break;
		case "sqlStatement":
			sqlStatement(request, response);
			break;
		case "sqlCreateTable":
			sqlCreateTable(request, response);
			break;
		case "sqlDropTable":
	...
	protected void sqlStatement(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		String sqlStatement = request.getParameter("sqlTxt");
		String msg;
		try {
			new CardDao().sql(sqlStatement);
			msg = "SQL efetuado";
		} catch (Exception ex) {
			ex.printStackTrace();
			msg = "Erro: " + ex.getLocalizedMessage();
	...
	protected void sqlCreateTable(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		String sqlStatement = "CREATE TABLE card(cardId INTEGER GENERATED BY DEFAULT AS IDENTITY(START WITH 0) NOT NULL PRIMARY KEY, "
							+ "cardWord VARCHAR(256) NOT NULL, censuredWords VARCHAR(256) NOT NULL);";
		String msg;
		try {
			new CardDao().sql(sqlStatement);
			msg = "SQL efetuado";
	...
	protected void sqlDropTable(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		String sqlStatement = "DROP TABLE card;";
		String msg;
		try {
			new CardDao().sql(sqlStatement);
			msg = "SQL efetuado";
